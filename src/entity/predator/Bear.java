...
public class Bear extends Predator {
    ...
    public Bear() {
        weight = Properties.WEIGHT_BEAR;
        stepDeath = 0;
        name = Integer.toString(newBear);
        satiety = Properties.SATIETY_BEAR;
        isEat = true;
        isMove = false;
        isReproduce = false;
    }
...
//
        ...
        if (bears.size() > Properties.MAX_BEAR) {
            needToKill = Math.abs(bears.size() - Properties.MAX_BEAR);
            while (needToKill > 0) {
                iterator.next();
                iterator.remove();
                Bear.deathBear++;
        ...
        if (animal.size() > 0) {
            ...
            if (object instanceof Duck) {
                ...
                if (this.isEat == false && this.isReproduce == false && this.isMove == false && (chanceToEat < 10)) {
                    ...
                    this.satiety = Properties.SATIETY_BEAR;
                    this.stepDeath = 0;
                    this.isEat = true;
                } else {
                    this.isEat = false;
                    this.satiety -= Properties.SATIETY_STEP_TO_DEATH_MAX_BEAR;
