...
public class Wolf extends Animal {
    ...
    public Wolf() {
        weight = Properties.WEIGHT_WOLF;
        name = Integer.toString(newWolf);
        satiety = Properties.SATIETY_WOLF;
        isEat = true;
        isMove = false;
        isReproduce = false;
    }
...
//
        ...
        if (wolves.size() > Properties.MAX_WOLF) {
            needToKill = Math.abs(wolves.size() - Properties.MAX_WOLF);
            while (needToKill > 0) {
                iterator.next();
                iterator.remove();
                Wolf.deathWolf++;
        ...
        if (animal.size() > 0) {
            ...
            if (object instanceof Duck) {
                ...
                if (this.isEat == false && this.isReproduce == false && this.isMove == false && (chanceToEat < 40)) {
                    ...
                    this.satiety = Properties.SATIETY_WOLF;
                    this.stepDeath = 0;
                    this.isEat = true;
                } else {
                    this.isEat = false;
                    this.satiety -= Properties.SATIETY_STEP_TO_DEATH_MAX_WOLF;
