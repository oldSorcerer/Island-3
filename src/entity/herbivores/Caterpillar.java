...
public class Caterpillar extends Herbivores {
    ...
    public Caterpillar() {
        weight = Properties.WEIGHT_CATERPILLAR;
        satiety = Properties.SATIETY_CATERPILLAR;
        isEat = true;
        isMove = false;
        isReproduce = false;
    ...
    public static void life(Cell[][] cells, int i, int j) {
        ...
        if (caterpillars.size() > Properties.MAX_CATERPILLAR) {
            needToKill = Math.abs(caterpillars.size() - Properties.MAX_CATERPILLAR);
            while (needToKill > 0) {
                iterator.next();
                iterator.remove();
    ...
    public void eat(Plant plant) {
        if (plant.getWeight() > Properties.SATIETY_CATERPILLAR && this.isEat == false && this.isReproduce == false) {
            plant.setWeight(plant.getWeight() - Properties.SATIETY_CATERPILLAR);
            this.satiety = Properties.SATIETY_CATERPILLAR;
            this.isEat = true;
        } else {
            this.isEat = false;
        }
